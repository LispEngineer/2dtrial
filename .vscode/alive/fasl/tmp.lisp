...
;(defmethod trial:setup-scene ((main my-game) scene) 

  ; ;; Set up the UI
  ; (let* ((ui (make-instance 'trial-alloy:ui))
  ;        (layout-tree (alloy:layout-tree ui))
  ;        (focus-tree (alloy:focus-tree ui))
  ;        (main-layout (make-instance 'alloy:border-layout))
  ;        (focus-list (make-instance 'alloy:focus-list))
  ;        (panel (make-instance 'right-panel :cell-margins (alloy:margins 10))))

  ;   (let ((rows '(("Health: 100"  0.0 1.0 0.0)  ; Green
  ;                 ("Mana: 50"     0.0 0.0 1.0)  ; Blue
  ;                 ("Gold: 250"    1.0 1.0 0.0)  ; Yellow
  ;                 ("Class: Mage"  1.0 0.0 1.0)  ; Magenta
  ;                 ("Zone: Start"  1.0 1.0 1.0)))) ; White
  ;     (loop for (text r g b) in rows
  ;           ;; Create an RGBA color object using colored:color
  ;           for current-color = (chroma:color r g b 1.0)
  ;           for data = (make-instance 'alloy:value-data :value text)
  ;           for label = (make-instance 'colored-label :data data :text-color current-color)
  ;           do (alloy:enter label panel)
  ;              (alloy:enter label focus-list)))

  ;   (alloy:enter panel main-layout :place :east :size (alloy:vw 0.2))

  ;   (alloy:enter main-layout layout-tree)
  ;   (alloy:enter focus-list focus-tree)
